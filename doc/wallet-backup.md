...
Backing up your Bitzec private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
For all methods, you will need to include an export directory setting in your config file (`bitzec.conf` located in the data directory which is `~/.zcash/` unless it's been overridden with `datadir=` setting):
